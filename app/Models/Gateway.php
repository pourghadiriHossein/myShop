...
class Gateway
{
    private function send($orderID, $price)
    {
        $params = array(
            'order_id' => $orderID,
            'amount' => $price,
            ...
            'mail' => null,
            'desc' => null,
            'callback' => 'http://localhost/myShop/public/callback',
        );
    ...
    private function verify($IDPayID, $orderID)
    {
        $params = array(
            'id' => $IDPayID,
            'order_id' => $orderID,
    ...
    public function start($orderID, $price)
    {
        $result = $this->send($orderID, $price);
        $result = json_decode($result);
        return $result;
    }

    public function done($IDPayID, $orderID)
    {
        $result = $this->verify($IDPayID, $orderID);
        $result = json_decode($result);
        return $result;
    }
