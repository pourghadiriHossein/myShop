                ...
                <div class="col-lg-12">
                    <section class="panel">
                        ...
                        <div class="panel-body">
                            ...
                            <form class="form-horizontal" action="{{ route('adminPostUpdateCategory',$selectedCategory->id) }}" method="post" enctype="multipart/form-data">
                                {{csrf_field()}}
                                <fieldset title="اطلاعات پایه" class="step" id="default-step-0">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-10">
                                            <select name="parent_id" class="form-control" style="height: 40px">
                                                <option value="{{ null }}" selected>دسته اصلی</option>
                                                @foreach($allCategory as $category)
                                                <option value="{{ $category->id }}" @if($category->id == $selectedCategory->parent_id) selected @endif>{{ $category->label }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-10">
                                            <input value="{{ $selectedCategory->label }}" type="text" required="required" oninvalid="this.setCustomValidity('نمیتواند خالی باشد')"
                                                   onchange="this.setCustomValidity('')" name="label" class="form-control" placeholder="نام دسته">
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-10">
                                            <select name="status" class="form-control" style="height: 40px">
                                                <option value="0" @if($selectedCategory->status == 0) selected @endif>غیر فعال</option>
                                                <option value="1" @if($selectedCategory->status == 1) selected @endif>فعال</option>
                                            </select>
                                        </div>
                                    </div>
