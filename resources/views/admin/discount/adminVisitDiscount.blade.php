                    ...
                    <div   class="col-xs-12 col-sm-12 col-md-12 table-responsive">
                        ...
                        <table id="orderTable" class="table table-striped">
                            ...
                            <tr>
                                ...
                                <td>@if($discount->price == null) ندارد @else {{ $discount->price }} ريال @endif</td>
                                <<td>@if($discount->percent == null) ندارد @else {{ $discount->percent }} درصد @endif</td>
                               <td>@if($discount->token == null) ندارد @else {{ $discount->token }} @endif</td>
                               <td>
                                    @if($discount->status == 0)<p class="label label-warning" style="width: 250px">غیر فعال</p>
                                    @else
                                        <p class="label label-success" style="width: 250px">فعال</p>
                                    @endif
                                </td>
                                <td>
                                    <a class="label label-warning" href="{{route('adminUpdateDiscount',$discount->id)}}">ویرایش</a>
                                </td>
                            </tr>
                            @endforeach
                            </tbody>
