                    ...
                    <div   class="col-xs-12 col-sm-12 col-md-12 table-responsive">
                        ...
                        <table id="orderTable" class="table table-striped">
                            ...
                            <tr>
                                ...
                                <td>@if($order->discount_id == null)تخفیف ندارد @else
                                 @if($order->discount->price){{ $order->discount->price }},@endif
                                 @if($order->discount->percent){{ $order->discount->percent }},@endif
                                 @if($order->discount->token){{ $order->discount->token }},@endif
                                @endif</td>
                                <td>{{ $order->total_price }} ریال</td>
                                <td>{{ $order->pay_price }} ریال</td>
                                <td>
                                    @if($order->status == 0)<p class="label label-warning" style="width: 250px">غیر فعال</p>@endif
                                    @if($order->status == 1)<p class="label label-success" style="width: 250px">فعال</p>@endif
                                </td>
                                <td>
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <table id="orderTable" class="table table-striped">
                                                        ...
                                                        @foreach($order->orderListItems as $OLI)
                                                            <tr>
                                                                <td>{{ $OLI->id }}</td>
                                                                <td><img src="{{asset(\App\Models\Tool::getProductImage($OLI->product_id))}}" height="50" width="40"></td>
                                                ...
                                                <div class="modal-footer">
                                                    <button data-dismiss="modal" class="btn btn-warning" type="button">بستن</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
